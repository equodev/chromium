...
/**
 * A <code>WindowEvent</code> is sent by a {@link Browser} when a new window
 * needs to be created or when an existing window needs to be closed. This
 * notification occurs when a javascript command such as
 * <code>window.open</code> or <code>window.close</code> gets executed by a
 * <code>Browser</code>.
 * 
 * <p>The following example shows how <code>WindowEvent</code>'s are typically
 * handled. <code><pre>
 ...
 *    });
 *  }
 * </pre></code> The following notifications are emitted when the user selects a
 * hyperlink that targets a new window or as the result of a javascript that
 * executes window.open.
 * 
 * <p>Main Browser
 * <ul>
 * <li>User selects a link that opens in a new window or javascript requests a
 * new window</li>
 * <li>OpenWindowListener.open() notified</li>
 * <ul>
 * <li>Application creates a new Shell and a second Browser inside that
 * Shell</li>
 * <li>Application registers WindowListener's on that second Browser, such as
 * VisibilityWindowListener</li>
 * <li>Application returns the second Browser as the host for the new window
 * content</li>
 * </ul>
 * </ul>
 * 
...
public class WindowEvent extends TypedEvent {
  ...
  public String toString() {
    ...
    return string.substring(0, string.length() - 1) // remove trailing '}'
        + " required=" + required + " browser=" + browser + " location=" + location + " size="
        + size + " addressBar=" + addressBar + " menuBar=" + menuBar + " statusBar=" + statusBar
        + " toolBar=" + toolBar + "}";
  }
}
