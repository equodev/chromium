...
public class Chromium1 extends Composite {
...
//private ScheduledFuture<?> pumpTask;
	...
	public Chromium1(Composite parent, int style) {
...
//		hwnd = embeddedHandle;
		...
		focusHandler.setOnTakeFocus((focusHandler, browser_1, next) -> {
			...
			int indexOf = Arrays.asList(tabOrder).indexOf(Chromium1.this);
			if (indexOf != -1) {
...
//				System.err.println("Ignore do_message_loop_work due inactive shell");
	...
	private static void fixJNRClosureClassLoader() {
		try {
			...
			parent.set(asmClassLoader, Chromium1.class.getClassLoader());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
